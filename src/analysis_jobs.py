...
# Модель для фоновых задач (анализ/аудит). Sonnet — ~5× дешевле Opus при рабочем
# качестве на разборе с браузером. Переопределяется ANALYSIS_MODEL в .env.
...
def _build_competitor_prompt(targets: str, focus: str, out_path: Path) -> str:
    ...
    return (
        ...
        "РАМКИ (важно — экономим токены, работаем фокусно):\n"
        "• Бери ДО 6–8 самых заметных/релевантных конкурентов в нише, не пытайся охватить "
        "всех подряд. Лучше точнее по немногим, чем поверхностно по десяткам.\n"
        "• С каждой страницы снимай ТОЛЬКО поля оси сравнения. Не вытягивай и не цитируй "
        "страницу целиком, не копируй длинные куски — держи контекст лёгким.\n"
        "• Браузер (playwright) открывай ТОЧЕЧНО — лишь когда WebFetch/WebSearch реально не "
        "дали факт (JS-цена, 403). Не открывай страницу ради того, что уже видно из поиска.\n"
        "• Ответ — сжатый и по делу, без воды и длинных пересказов.\n\n"
        "Как работать:\n"
...
def _build_audit_prompt(url: str, focus: str, out_path: Path) -> str:
    ...
    return (
        ...
        "РАМКИ (экономим токены): сними страницу ОДИН раз и работай по снимку; не "
        "перезагружай браузер без нужды. В контекст тяни только нужные блоки, не "
        "всю разметку. Ответ — сжатый, по делу, без длинных пересказов страницы.\n\n"
        "Как работать:\n"
