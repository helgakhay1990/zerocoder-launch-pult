...
- **Размер:** **2 ядра / 4 ГБ** — проверено на проде (`193.43.72.107`, 23.06): на 4 ГБ помещается весь бот целиком, включая монтаж (whisper модель `base`), анализ и playwright. На 961 МБ (как давали изначально) тяжёлые функции не лезли — 4 ГБ это рабочий минимум. Нужен запас под модели/видео (`agent-assistant/models`, `source/`). Монтаж по записи в часы на 4 ГБ идёт, но не быстро; если монтаж на сервере не нужен — можно держать его локально и не ставить whisper.
...
- **Монтаж по времени.** whisper по записи в часы идёт минутами (модель `base`); на 4 ГБ работает (проверено на проде), на совсем слабом VPS — долго. Если монтаж на сервере не нужен — не ставить whisper, держать монтаж на локальной машине.
