...
> ℹ️ **Текущий статус — локальный MVP.** Бот запускается на личном компьютере и работает, **только пока этот компьютер включён, есть интернет и запущен процесс бота**. Это не облачный сервис 24/7. Причина — «мозг» бота это `claude -p`, который исполняется на машине с авторизованным Claude Code. Перевод в режим 24/7 (сервер + автозапуск) описан в [`docs/DEPLOY.md`](docs/DEPLOY.md) — это следующий шаг развития, в MVP не входит.
